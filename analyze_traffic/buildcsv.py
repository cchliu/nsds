...
def worker(sublist, indir, pcap_dir, outdir, out_q, thread_id):
	count = 0 
	lenth = len(sublist)
	for tmp_file in sublist:
		...
		pcap_file_path = os.path.join(pcap_dir, os.path.relpath(tmp_file, indir))
		...
		#file_name = os.path.join(outdir, os.path.basename(tmp_file))
...
def main(indir, pcap_dir, outdir):
	...
	for i in range(numproc):
		...
		p = Process(target = worker, args = (sublist, indir, pcap_dir, outdir, out_q, i))
		procs.append(p)
		p.start()
